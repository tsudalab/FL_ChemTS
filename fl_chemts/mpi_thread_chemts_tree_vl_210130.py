...
def ChemTS_run(rootnode,result_queue,lock,chem_model,ts_strategy,search_parameter,num_simulations, gau_parallel,simulation_time, output_file,alpha,objective,num_rollout,charge_check,SA_score_check):
	...
	#lock.release()
        ...
        #output
        print('End-of-loop')
        print('Generated:', str(wave_compounds[-1]), wave[-1], s1_wavelength_list[-1])
        with open('/home/terayama/csvcom_.csv','w') as file: 
            for line1 in wave_compounds:
                file.write(str(line1))
                file.write('\n')
        
        with open('/home/terayama/csvwave_.csv','w') as file:
            for line2 in wave:
                file.write(str(line2))
                file.write('\n')
                        
        with open('/home/terayama/chem/'+output_file,'w') as file:
